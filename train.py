...
for epoch in range(start_epoch, NUM_EPOCHS):
    ...
    for i,(inputs,ema_inputs,labels) in enumerate(val_loader):
    	...
    	consistency_eval_loss = consistency_criterion(model_logit, ema_model_logit) / batch_size
    	...
    	running_eval_consistency_loss += get_current_consistency_weight(epoch)*consistency_eval_loss.item() * batch_size
    ...
    with open(r'sre/train_log.txt', 'a+') as f:
    	f.write("EPOCH: {} \n".format(epoch))
    	f.write('Total loss {} \n'.format(epoch_loss))
    	f.write("class loss {}".format(epoch_class_loss))
    	f.write("ema class loss {}".format(epoch_class_loss))
    	f.write('consistency loss {}'.format(epoch_cons_loss))
    	f.write('Total val loss {}'.format(vepoch_loss))
    	f.write("class val loss {}".format(vepoch_class_loss))
    	f.write("ema class val loss {}".format(vepoch_ema_class_loss))
    	f.write('consistency val loss {}'.format(vepoch_cons_loss))
    	f.write('AUC score: {}'.format(auc_score))
    	f.write("\n")
    	f.write("\n")
